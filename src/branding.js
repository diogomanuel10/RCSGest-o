...
// CLARAS (--navy-100/-300, usadas como fundos suaves) derivam para branco.
function applyPalette(primary, accent) {
  ...
  const softTint = '#ffffff';

  s.setProperty('--navy', p);
  s.setProperty('--navy-700', mix(p, '#000000', 0.22));
  s.setProperty('--navy-500', mix(p, '#ffffff', 0.16));
  s.setProperty('--navy-300', mix(p, softTint, 0.34));
  s.setProperty('--navy-100', mix(p, softTint, 0.88));
